            ...
            <table className="min-w-full divide-y divide-slate-200 relative">
              <thead className="bg-slate-100 dark:bg-slate-800/50 sticky top-0 z-10 shadow-sm">
                <tr>
                  ...
                  <th className="px-2 py-3 text-center text-sm font-bold text-slate-700 dark:text-slate-200 border-l bborder-slate-200 dark:border-slate-700">เข้าบ่าย</th>
                  ...
                  <th className="px-2 py-3 text-center text-sm font-bold text-orange-700 bg-orange-50/50 border-l border-orange-200 min-w-[110px]">ตัวคูณ OT</th>
                  <th className="px-2 py-3 text-center text-sm font-bold text-emerald-700 bg-emerald-50/50 border-l border-emerald-200 min-w-[110px]">เงินเพิ่ม (฿)</th>
                  <th className="px-2 py-3 text-center text-sm font-bold text-rose-700 bg-rose-50/50 border-l border-rose-200 min-w-[110px]">หักเงิน (฿)</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={emp.id} className={`hover:bg-slate-50 dark:bg-slate-900 transition ${emp.is_edited ? "bg-yellow-50" : ""} ${isMissingPunch ? "bg-red-50" : ""} ${isAbsent ? "opacity-50" : ""}`}>
                      ...
                      {["time_in_4", "time_out_4"].map((field, idx) => (
                        <td key={field} className={`px-1 py-2 text-center bg-indigo-50/20 ${idx === 0 ? "border-l border-indigo-100" : ""}`}>
                          <ThaiTimeInput
                            value={emp[field]}
                            onChange={(val) => handleInputChange(emp.id, field, val)}
                            theme="indigo"
                      ...
                      <td className="px-2 py-2 text-center border-l border-orange-100 bg-orange-50/10">
                        <select
                          ...
                          className={`w-full px-2 py-1.5 text-xs font-bold border rounded-lg focus:ring-2 focus:ring-orange-500 outline-none transition shadow-sm cursor-pointer ${Number(emp.ot_multiplier) > 1.0 ? 'bg-orange-100 text-orange-800 border-orange-300' : 'bg-slate-50 text-slate-700 border-slate-200'}`}
                        >
                      ...
                      <td className="px-2 py-2 text-center border-l border-emerald-100 bg-emerald-50/10">
                        <input
                          ...
                          className="w-full min-w-[60px] px-2 py-1.5 text-xs font-bold border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none transition shadow-sm bg-slate-50 text-slate-700 border-slate-200"
                        />
                      </td>
                      {/* หักเงิน */}
                      <td className="px-2 py-2 text-center border-l border-rose-100 bg-rose-50/10">
                        <input
                          ...
                          className="w-full min-w-[60px] px-2 py-1.5 text-xs font-bold border rounded-lg focus:ring-2 focus:ring-rose-500 outline-none transition shadow-sm bg-slate-50 text-slate-700 border-slate-200"
                        />
                      </td>
                    </tr>
                  );
