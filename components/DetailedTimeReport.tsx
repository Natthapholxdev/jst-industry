...
export default function DetailedTimeReport() {
  ...
  const getDetailedLines = (log: any) => {
    ...
    // Check missing scans or times
    if (log.time_in_1 || log.time_out_1) {
      lines.push(`${log.time_in_1 || '?'} - ${log.time_out_1 || '?'}`);
    }
  ...
  const exportToExcel = () => {
    ...
    xlsx.utils.book_append_sheet(workbook, worksheet, 'รายงานเวลาเข้าออก');

    const excelBuffer = xlsx.write(workbook, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8' });
    
    saveAs(data, `รายงานเวลาแบบละเอียด_${startDate}_ถึง_${endDate}.xlsx`);
  };
